...
exports._transformNode = function (node) {
  ...
  declarations.forEach(function defineAndAttachTokens (declaration, index) {
    ...
    if (index < declarations.length - 1) {
      ...
      if (!betweenNewlineToken) {
        ...
        var newNewlineToken = exports.cloneToken(lineBreakToken);
      ...
      if (endingSemicolonToken) {
        ...
        // DEV: There cannot already be a semicolon as that would make for different statements
        ...
        firstBetweenToken.prev = newSemicolonToken;
        newSemicolonToken.next = firstBetweenToken;

        // Swap places with the first firstBetweenToken
        firstBetweenToken = newSemicolonToken;
      // Otherwise, if there is still a comma (e.g. there was a new line so, ASI would kick in)
      } else if (betweenNewlineToken) {
        // Remove the comma to have matching line endings (e.g. semicolon-less)
        var betweenCommaToken = rocamboleToken.findInBetween(firstBetweenToken, lastBetweenToken.next, rocamboleToken.isComma);
        betweenCommaToken.prev.next = betweenCommaToken.next;
        betweenCommaToken.next.prev = betweenCommaToken.prev;
      }
  ...
  // Connect our VariableDeclaration tokens
  declarations.forEach(function connectDeclarationTokens (declaration, index) {
    // If this isn't the first, then connect the previous to the current one
    if (index > 0) {
      var lastDeclaration = declarations[index - 1];
      lastDeclaration.endToken.next = declaration.startToken;
      declaration.startToken.prev = lastDeclaration.endToken;
    }
  });

  // Walk over our declarations and fix up their indentation to be consistent with the first one's
  declarations.forEach(function alignIndent (declaration, index) {
    // If this is the non-first declaration (as we trust the existing `var` indent)
    if (index !== 0) {
      // Find the line break token before our declaration
      var prevLineBreakToken = rocamboleToken.findPrev(declaration.startToken, rocamboleToken.isBr);

      // Find the first non-empty token (e.g. inline comment or `var`)
      var firstNonEmptyToken = rocamboleToken.findNext(prevLineBreakToken, rocamboleToken.isNotEmpty);

      // Replace the whitespace with our prestarting variation
      var newPreStartingTokens = exports.cloneTokenChain(preStartingTokens);
      if (newPreStartingTokens.length) {
        prevLineBreakToken.next = newPreStartingTokens[0];
        newPreStartingTokens[0].prev = prevLineBreakToken;
        firstNonEmptyToken.prev = newPreStartingTokens[newPreStartingTokens.length - 1];
        newPreStartingTokens[newPreStartingTokens.length - 1].next = firstNonEmptyToken;
      } else {
        prevLineBreakToken.next = firstNonEmptyToken;
        firstNonEmptyToken.prev = prevLineBreakToken;
      }
    }
  });

  console.log('FINAL', node.startToken.root.toString());
