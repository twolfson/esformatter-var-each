...
exports.nodeBefore = function (node) {
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    declarations.forEach(function splitDeclarations (declaration, index) {
      // If we are the last declaration, do nothing
      var nextDeclaration = declaration.next;
      if (nextDeclaration === undefined) {
        return;
      }

      // Make our declaration standalone
      delete declaration.next;
      console.log(declaration.endToken, declaration.endToken);

      // Promote the next declaration to
    });
  }
