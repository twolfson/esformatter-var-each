...
exports._transformNode = function (node) {
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    declarations.forEach(function indentDeclaration (declaration, index) {
      ...
      var prevDeclarationEndToken = declaration[index - 1].endToken;
      var varStartToken = declaration.startToken;
      var token = varStartToken;
      var foundLineBreak = false;
      // while (token && token !== prevDeclarationEndToken) {

      // }

      // If there was not a line break before, add one
      // if (foundLineBreak === false) {
      ...
      if (varDeclaration.startToken.prev && varDeclaration.startToken.prev.type === 'Indent') {
        ...
        // TODO: This will prob need rework with semicolon-less
