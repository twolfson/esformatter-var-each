...
exports._transformNode = function (node) {
  ...
  declarations.forEach(function defineAndAttachTokens (declaration, index) {
    ...
    //     Substeps here: Attach FIRST `;\n  ` token to previous VariableDeclaration
    ...
    // If this is a non-first and a non-last declaration
    if (index > 0 && index < declarations.length - 1) {
      // SUBSTEP: Create `;\n  ` tokens
      var newBetweenTokens = betweenDeclarationTokens
    }
  });
