...
exports._transformNode = function (node) {
  ...
  // Additionally, find the whitespace tokens before our `var` started (e.g. all indents/first new line -- don't want multiple newlines)
  var preStartingTokens = [];
  var token = varDeclaration.startToken.prev, earlierNonEmptyToken;
  while (token) {
    ...
    if (token.type === 'WhiteSpace' || token.type === 'Indent') {
      ...
      token = token.prev
    // Otherwise, if the token is a line break, save it and stop
  ...
  declarators.forEach(function generateDeclaration (declarator, index) {
    ...
    if (index > 1) {
      ...
      var lineBreakBetweenDeclarators = rocamboleToken.findInBetween(
        lastDeclarator.endToken, declarator.startToken, rocambole.isBr);
    }
    ...
    // Otherwise, connect this to the previous declaration
      ...
      // If there's a newline between these declarations, then create a matching newline + indent

      // If there are prestarting sections (e.g. newlines/whitespace before a `var`), then build/attach them now
