...
exports._transformNode = function (node) {
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    // If we have been working with semicolons, correct our `var` indentation
    if (varDeclaration.endToken.value === ';') {
      // Detect the level of indentation
      var varIndentationToken = varDeclaration.startToken.prev;
      if (!varIndentationToken || varDeclaration.type !== 'Indent') {
        varIndentationToken = null;
      }
      ...
      declarations.forEach(function indentDeclaration (declaration, index) {
        ...
        if (varIndentationToken) {
          // Add indentation after the line break and before the `var`
          declaration.startToken.next = {
            type: varIndentationToken.type,
            value: varIndentationToken.value,
            prev: declaration.startToken,
            next: varStartToken
              // prev: declaration.startToken.next
          };
          varStartToken.prev = declaration.startToken.next;
        }
      });

      // For each of the declaration, unindent the remainder of its content
      // TODO: This dev is wrong they can go `function () {}, function () {} (no semic)`
      // DEV: Reminder that this doesn't run on `semicolon-less` content
      // DEV: but we are performing a noop for them as is so this blends nicely
      declarations.forEach(function unindentDeclarationContent (declaration) {
        // Walk across the tokens and unindent any indenation
        // TODO: This won't play nice with semicolons... prob need a ternary or something
        // TODO: Or maybe we perform unindentation before fucking with `var`?
        var token = declaration.startToken;
        var endToken = declaration.endToken;
        while (token && token !== endToken) {
          // If the token is indentation, decrease the depth and unindent
          if (token.type === 'Indent') {
            console.log(token.depth, token.value);
            console.log(options);
          }
