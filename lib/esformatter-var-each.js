  ...
  declarations.forEach(function defineAndAttachTokens (declaration, index) {
    ...
    if (index < declarations.length - 1) {
      ...
      if (endingSemicolonToken) {
        // If the next token is a comma, replace it
        // e.g. `var a,b;` -> `var a;b;`
        if (rocamboleToken.isComma(firstBetweenToken)) {
          // Copy the token chain
          var newSemicolonToken = exports.cloneToken(endingSemicolonToken);
          newSemicolonToken.prev = firstBetweenToken.prev;
          newSemicolonToken.next = firstBetweenToken.next;

          // Update the prev/next tokens to look at us (I'm the captain now)
          firstBetweenToken.prev.next = newSemicolonToken;
          firstBetweenToken.next.prev = newSemicolonToken;

          // Update the firstBetweenToken to be our new semicolon
          firstBetweenToken = newSemicolonToken;
        // Otherwise, insert it before the next token
        // e.g. LineBreak -- `var a,\nb;` -> `var a;\nb;`
      ...
      //   then insert it at the comma
      // e.g. `var a, b` -> `var a\n b`
      // e.g. `var a /* hi */, b` -> `var a /* hi */\n b`
      // e.g. `var a, /* hi */ b` -> `var a\n /* hi */ b`

      // Find the new line where our next declaration begins


      // Save the declaration's ending token
      // TODO: This isn't quite right either, it should be nextDeclaration.startToken.prev
