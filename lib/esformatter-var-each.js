...
exports._transformNode = function (node) {
  ...
  declarations.forEach(function defineAndAttachTokens (declaration, index) {
    ...
    //         Substeps here: Attach FIRST `var ` token AS current VariableDeclaration START token
    ...
    // STEP: Define STARTING tokens for each VariableDeclaration (e.g. `var ` for `var a = 1`)
    // SUBSTEP: If this is the first VariableDeclaration
    if (index === 0) {
      // SUBSTEP: Replace ORIGINAL VariableDeclaration linkage with FIRST VariableDeclaration to HEAD (e.g. reuse the existing `var ` token chain)
      // DEV: `firstDeclarator.prev` is already linked with all previous tokens in the application, making this transition easy
      // DEV: `firstDeclarator.prev` will be the whitespace of `var ` (i.e. `['var', ' ']`, it's the `' '`)
      var firstDeclarator = varDeclaration.declarations[index];
      declaration.startToken = firstDeclarator.prev;
    // SUBSTEP: Otherwise, (we are a non-first VariableDeclaration)
    } else {
      // SUBSTEP: Insert leading content for each non-first VariableDeclaration between VariableDeclaration's
      // SUBSTEP: Create `var ` tokens
      var newStartingTokens = exports.cloneTokenChain(startingTokens);

      // SUBSTEP: Attach FIRST `var ` token TO last VariableDeclaration END token
      var lastDeclaration = declarations[index - 1];
      // DEV: This is always defined as we always need a `var` keyword
      lastDeclaration.endToken.next = newStartingTokens[0];

      // SUBSTEP: Attach FIRST `var ` token AS current VariableDeclaration START token
      declaration.startToken = newStartingTokens[0];
    }

    // STEP: Define ENDING tokens for each VariableDeclaration (e.g. `;\n  ` for `var a = 1;\n  `)
    // SUBSTEP: If this is a non-last VariableDeclaration
    if (index !== declarations.length - 1) {
      // SUBSTEP: Insert terminating content for each VariableDeclaration between VariableDeclaration's (e.g. `;\n  `)
      // SUBSTEP: Create `;\n  ` tokens
...
console.log('wuuut', declaration.startToken);
