...
exports._transformNode = function (node) {
  ...
  declarations.forEach(function defineAndAttachTokens (declaration, index) {
    ...
    // TODO: Need to link everything from left to right/
    //   then, go in reverse and add backlinks and the loop won't work
    //   Otherwise, we get cycles
    ...
    // Otherwise, (we are a non-first VariableDeclaration)
      ...
      queue.push({
        srcNode: lastDeclaration,
        srcTokenKey: 'endToken',
        srcDirectionKey: 'next',
        targetToken: firstStartingToken,
        targetDirectionKey: 'prev'
      });
      declaration.startToken = firstStartingToken;
    ...
    if (index < declarations.length - 1) {
      ...
      // nextDeclaration.startToken.prev = lastBetweenToken;
      // lastBetweenToken.next = nextDeclaration.startToken;
      queue.push({
