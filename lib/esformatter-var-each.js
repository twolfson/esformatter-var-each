...
exports._transformNode = function (node) {
  ...
  var endingTokens = [];
  var firstLineBreakAfterLastDeclarator = rocamboleToken.findNext(declarators[declarators.length - 1].endToken, function isLineBreakOrEndToken (token) {
    return token.type === 'LineBreak' || token === varDeclaration.endToken;
  });
  rocamboleToken.eachInBetween(declarators[declarators.length - 1].endToken.next, firstLineBreakAfterLastDeclarator, function saveToken (token) {
    // DEV: We skip LineBreak as it causes insertion of multiple newlines if there are trailing newlines in a function
    //   e.g. `var a, b\n\n\n` -> `var a;\n\n\nvar b;\n\n\n`
  ...
  while (token) {
    ...
    //   so we need to rely on spacing between vars instead
    } else {
      break;
    }
  }

  // Calculate line breaks between declarators
  var lineBreakTokenBetweenLastDeclaratorArr = declarators.map(function getEndToken (declarator, index) {
    if (index > 0) {
      var lastDeclarator = declarators[index - 1];
      return rocamboleToken.findInBetween(
        lastDeclarator.endToken, declarator.startToken, rocamboleToken.isBr);
    }
    return null;
  });
  ...
  declarators.forEach(function generateDeclaration (declarator, index) {
    ...
    // Before we edit any tokens, determine if there's a line break between the last and current declarator
    var lineBreakTokenBetweenDeclarators = lineBreakTokenBetweenLastDeclaratorArr[index];
    ...
    //   Handle tokens
    // TODO: How do we handle the the else case for this?
    if (newStartingTokens.length) {
