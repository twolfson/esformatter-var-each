...
exports._transformNode = function (node) {
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    var declarations = declarators.map(function generateDeclaration (declarator, index) {
      ...
      // Add a whitespace token after our initial var
      rocamboleToken.after(declaration.startToken, {
        type: 'WhiteSpace',
        value: ' '
      });

      // If this is the first declarator, replace the initial `var`
      if (index === 0) {
        rocamboleToken.remove(varDeclaration.startToken);
        console.log('index', node.parent.toString());
      }

      // Connect our tokens
      rocamboleToken.after(declaration.startToken.next, declarator.startToken);
      rocamboleToken.after(declarator.endToken, declaration.endToken);

      console.log('current2:', node.parent.toString());
