  ...
  if (node.type === 'VariableDeclaration') {
    ...
    var declarations = declarators.map(function generateDeclaration (declarator, index) {
      ...
      var declaration = {
        ...
        endToken: {
          ...
          next: {
            // TODO: This will probably cause double line breaks so we need to fallback `next` resolution during linking or store it as a property
            type: 'LineBreak',
            value: '\n',
            next: {
              type: 'WhiteSpace',
            ...
            // TODO: Handle indentation to same `var` level
            // prev: set to `declaration.endToken`
    ...
    declarations.forEach(function connectDeclaration (declaration, index) {
      ...
      // Otherwise, connect to the next declaration
        ...
        declaration.endToken.next.next.next = varDeclarationEndToken.next;
      } else {
        // TODO: We might need whitespace... (e.g. `var hai;\nvar bai;`)
        declaration.next = declarations[index + 1];
        declaration.endToken.next.next.next = declarations[index + 1].startToken;
      }
      ...
      if (declaration.endToken.next.next.next) {
        declaration.endToken.next.next.next.prev = declaration.endToken;
      }
    });
