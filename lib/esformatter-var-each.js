...
exports._transformNode = function (node) {
  ...
  declarators.forEach(function generateDeclaration (declarator, index) {
    ...
    var lineBreakTokenBetweenDeclarators = false;
    if (index > 0) {
      var lastDeclarator = declarators[index - 1];
      lineBreakTokenBetweenDeclarators = rocamboleToken.findInBetween(
        lastDeclarator.endToken, declarator.startToken, rocamboleToken.isBr);
      console.log(lastDeclarator.endToken.next.next, lineBreakTokenBetweenDeclarators);
    }
    ...
    // Otherwise, connect this to the previous declaration
      ...
      if (lineBreakTokenBetweenDeclarators) {
        ...
        rocamboleToken.before(linkingStartToken, lineBreakToken);
        console.log(lineBreakToken);
      }
